...
class TestWrittenCalcExerciseBox(unittest.TestCase):
    ...
    def test_generate_box_content(self):
        ...
        self.assertGreater(
            len(texts), 0, "No text elements found in the generated box."
        )
    ...
    def test_values_in_box(self):
        ...
        for value in self.values:
            value_found = any(
                text.text in list("123456") for text in texts if text.text
            )
            self.assertTrue(
                value_found, f"Value {value} not found in the generated box."
            )
